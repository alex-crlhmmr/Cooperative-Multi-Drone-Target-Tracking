"""V3 Phase 1: SpreadEnv — spread drones from cluster for good bearing geometry.
...
class SpreadEnv(gym.Env):
    ...
    def _compute_reward(self, result: dict) -> np.ndarray:
        ...
        Each drone gets rewarded based on how far it is from its nearest neighbor.
        This gives individual gradient signal (fly away from closest drone).
        Cooperative but with differentiated reward.
        """
        drone_positions = result["drone_positions"]

        # Per-drone: distance to nearest neighbor
        ...
        for i in range(self.N):
            ...
            # Reward: log scale so early spreading gives strong signal
            # log(1 + dist/10) gives ~0.7 at 10m, ~1.8 at 50m, ~2.4 at 100m
