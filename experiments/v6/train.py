"""V6 Training — Single script: heuristic spread + RL residual on angular-repulsion tracking.

Phase 1: heuristic repulsion spread (400 steps, NOT learned)
Phase 2: RL residual on angular-repulsion tracking (learned)
"""
...
# ── SubprocVecEnv with Phase 1 warm-start ──
...
class _TrackEnvFactory:
    ...
    def __call__(self):
        return _WarmStartTrackEnv(self.cfg, seed=self.seed)


class _WarmStartTrackEnv:
    """Wrapper that runs heuristic spread before each tracking reset.

    Defers env creation to first reset() to avoid PyBullet deadlocks in
    forked subprocesses.
    """
    ...
    def _lazy_init(self):
        if self._initialized:
            return
        from experiments.v6.spread_env import SpreadEnv
        from experiments.v6.track_env import TrackEnv
        ...
        for step in range(self.cfg.spread_steps):
            ...
            for i in range(N):
                vel = repulsion_controller(
                    ...
                    gain=self.cfg.spread_gain,
                    target_pos=target_pos,
                    surround_weight=self.cfg.spread_surround_weight,
                )
...
def run_eval(actor, critic, cfg, device, obs_normalizer=None, num_episodes=5):
    ...
    for ep in range(num_episodes):
        env = _WarmStartTrackEnv(cfg, seed=10000 + ep)
        obs, _ = env.reset()
        ep_tr_P, ep_rmse = [], []
...
def train(cfg: V6Config):
    ...
    print(f"[V6] Training: {cfg.track_training_steps} steps, {E} envs, "
          f"angular_weight={cfg.angular_weight}, angular_gain={cfg.angular_gain}")
    t_start = time.time()
    num_rollouts = 0
    ...
    while total_steps < cfg.track_training_steps:
        ...
        # Eval
        if total_steps % cfg.eval_freq < E * N * rollout_steps:
        ...
        # Save periodic
        if total_steps % cfg.save_freq < E * N * rollout_steps:
            _save_checkpoint(save_path, actor, critic, actor_optimizer, critic_optimizer,
                             obs_normalizer, value_normalizer, total_steps, history,
    ...
    writer.close()
    elapsed = time.time() - t_start
    print(f"[V6] Training complete: {total_steps} steps, "
...
def parse_args():
    parser = argparse.ArgumentParser(description="V6: Train angular-repulsion tracking + RL residual")
    parser.add_argument("--steps", type=int, default=None)
    parser.add_argument("--num-envs", type=int, default=None)
    parser.add_argument("--lr", type=float, default=None)
    parser.add_argument("--angular-weight", type=float, default=None)
    parser.add_argument("--angular-gain", type=float, default=None)
...
def main():
    ...
    if args.lr is not None:
        cfg.lr = args.lr
    if args.angular_weight is not None:
        cfg.angular_weight = args.angular_weight
    if args.angular_gain is not None:
        cfg.angular_gain = args.angular_gain
    if args.seed is not None:
        cfg.seed = args.seed
    if args.tag is not None:
        cfg.save_path = f"./output/v6_{args.tag}"
