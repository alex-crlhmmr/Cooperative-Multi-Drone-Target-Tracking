"""V4 Evaluation — compare 3 scenarios side-by-side on cluster spawn."""
...
def _run_rl(spread_env, track_env, cfg, spread_steps, track_steps, seed,
            ...
            device=None, **kwargs):
    """V4 RL: learned spread → learned track."""
    spread_env.reset(seed=seed)
...
def main():
    parser = argparse.ArgumentParser(description="V4 Evaluation")
    ...
    if not args.baseline_only:
        # ── Scenario 3: V4 RL two-phase ──
        if args.spread_checkpoint and args.track_checkpoint:
            print()
            print("=" * 60)
            print("Scenario 3: V4 RL spread (200 steps) → RL track")
            print("=" * 60)
            ...
            r = run_scenario(
                "V4 RL (spread→track)", cfg, args.episodes,
            ...
            print("\nSkipping V4 RL scenario (no checkpoints provided)")
