"""V4 Phase 2 Training — MAPPO on TrackEnv with Phase 1 warm-start.

Changes from V3:
- Per-minibatch advantage re-normalization
- Entropy logged to CSV
- Uses V4Config (lr=3e-4, gamma=0.995, gae_lambda=0.97, etc.)
"""
...
class _WarmStartTrackEnv:
    ...
    def _lazy_init(self):
        ...
        if self._initialized:
            return
        from experiments.v4.spread_env import SpreadEnv
        from experiments.v4.track_env import TrackEnv
...
def train(cfg: V4Config, spread_mode: str = "repulsion",
    ...
    print(f"[V4 Track] Training: {cfg.track_training_steps} steps, {E} envs, "
          f"spread_mode={spread_mode}")
    print(f"[V4 Track] Hyperparams: lr={cfg.lr}, gamma={cfg.gamma}, "
    ...
    while total_steps < cfg.track_training_steps:
        ...
        if total_steps % cfg.eval_freq < E * N * cfg.rollout_steps:
            ...
            # CSV logging (with entropy column)
            csv_path = os.path.join(save_path, "eval_results.csv")
            write_header = not os.path.exists(csv_path)
            with open(csv_path, "a") as f:
                if write_header:
                    f.write("step,tr_P,rmse,reward,entropy\n")
                recent_r = np.mean(ep_rewards[-10:]) if ep_rewards else 0
    ...
    writer.close()
    elapsed = time.time() - t_start
    print(f"[V4 Track] Training complete: {total_steps} steps, "
...
def parse_args():
    parser = argparse.ArgumentParser(description="V4 Phase 2: Train Track Policy")
...
def main():
    ...
    if args.tag is not None:
        cfg.save_path = f"./output/v4_{args.tag}"
