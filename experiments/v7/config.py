"""V7 Config — RL residual spread with angular separation reward."""
...
class V7Config:
    ...
    spread_residual_scale: float = 0.3       # RL can nudge up to 30% of v_max
    spread_repulsion_gain: float = 1.0       # base repulsion strength
    spread_surround_weight: float = 0.3      # tangential orbit bias
    spread_obs_dim: int = 15                 # expanded obs for spread
    spread_critic_extra: int = 6

    # ── Spread reward weights ──
    reward_min_angle: float = 1.0            # maximize min pairwise angle
    reward_mean_angle: float = 0.3           # bonus for overall angular diversity
    reward_range: float = 0.2               # stay in [40, 120]m from target
    reward_range_min: float = 40.0
    reward_range_max: float = 120.0
    ...
    rollout_steps: int = 400                 # = spread_steps (full episode per rollout)
    ...
    # ── Training ──
    spread_training_steps: int = 3_000_000
    eval_freq: int = 50_000
    save_freq: int = 100_000
    save_path: str = "./output/v7"
    normalize_obs: bool = True
    normalize_value_targets: bool = True

    @property
    def spread_critic_obs_dim(self) -> int:
        return self.spread_obs_dim + self.spread_critic_extra  # 15 + 6 = 21
