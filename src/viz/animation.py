...
def animate_rl_tracking(
    drone_positions: np.ndarray,
    target_true_states: np.ndarray,
    consensus_est: np.ndarray,
    local_estimates: np.ndarray,
    adjacency: np.ndarray,
    tr_P_history: np.ndarray,
    measurements: list | None = None,
    active_edges: list | None = None,
    dt: float = 1.0,
    interval_ms: int = 50,
    title: str = "RL Policy Replay",
    trail_len: int = 80,
    plot_box: float | None = None,
    topology_name: str = "full",
    controller_label: str = "RL Policy",
):
    """Animate 3D RL-controlled tracking with per-drone estimates and angular separation.

    Same visual style as animate_consensus_tracking but for RL policy evaluation.
    Shows observer drones, comm links, local estimates, consensus estimate,
    true target, bearing lines, and side panel with tr(P) and angular separation.

    Args:
        drone_positions: (T, num_drones, 3)
        target_true_states: (T, 6) ground truth [px, py, pz, vx, vy, vz]
        consensus_est: (T, 6)
        local_estimates: (num_drones, T, 6)
        adjacency: (num_drones, num_drones)
        tr_P_history: (T,) trace of position covariance
        measurements: list of per-drone measurement lists per step
        active_edges: list of adjacency matrices per step
        controller_label: e.g. "RL Policy" or "Baseline"
    """
    T = target_true_states.shape[0]
    num_drones = drone_positions.shape[1]
    target_traj = target_true_states[:, :3]
    ...
    fig = plt.figure(figsize=(22, 13))
    ax3d = fig.add_axes([0.02, 0.18, 0.58, 0.77], projection='3d')
    ax_info = fig.add_axes([0.62, 0.18, 0.37, 0.77])
    ax_info.axis('off')
    ...
    for i in range(num_drones):
        c = drone_colors[i % len(drone_colors)]
        dot, = ax3d.plot([], [], [], 'o', color=c, markersize=9, zorder=4, alpha=0.9)
        trail, = ax3d.plot([], [], [], color=c, linewidth=1.0, alpha=0.25)
        obs_dots.append(dot)
        obs_trails.append(trail)

    # Comm links
    comm_lines, comm_pairs = [], []
    for i in range(num_drones):
        for j in range(i + 1, num_drones):
            if adjacency[i, j]:
                line, = ax3d.plot([], [], [], color=comm_color, linewidth=1.8,
                                  alpha=0.5, linestyle='-', zorder=2)
                comm_lines.append(line)
                comm_pairs.append((i, j))
    if comm_lines:
        ax3d.plot([], [], [], color=comm_color, linewidth=2, label='Comm active')
        ax3d.plot([], [], [], color=dropped_color, linewidth=1.2, linestyle='--',
                  alpha=0.4, label='Comm dropped')
    ...
    for i in range(num_drones):
        c = drone_colors[i % len(drone_colors)]
        dot, = ax3d.plot([], [], [], 'D', color=c, markersize=5, alpha=0.6, zorder=3)
        local_dots.append(dot)
    ax3d.plot([], [], [], 'D', color='#888888', markersize=5, label='Local estimates')

    target_dot, = ax3d.plot([], [], [], 'o', color=target_color,
                            markersize=14, label='True Target', zorder=6)
    target_trail_line, = ax3d.plot([], [], [], color=target_color,
                                    linewidth=2.5, alpha=0.5, linestyle='--')

    cons_dot, = ax3d.plot([], [], [], 's', color=consensus_color, markersize=11,
                          label='Consensus Avg', zorder=5)
    cons_trail, = ax3d.plot([], [], [], color=consensus_color, linewidth=1.5,
                             alpha=0.5, linestyle='--')

    arrow_frac = 0.3
    gimbal_true, gimbal_meas = [], []
    for i in range(num_drones):
        tl, = ax3d.plot([], [], [], color='#2ECC40', linewidth=1.5, alpha=0.5)
        ml, = ax3d.plot([], [], [], color='#FF4136', linewidth=1.5, alpha=0.5)
        gimbal_true.append(tl)
        gimbal_meas.append(ml)
    ax3d.plot([], [], [], color='#2ECC40', linewidth=2, label='True bearing')
    ax3d.plot([], [], [], color='#FF4136', linewidth=2, label='Measured bearing')
    ...
    ax3d.set_zlim(max(0, mid[2] - half), mid[2] + half)
    ...
    ax3d.legend(loc='lower right', fontsize=7, framealpha=0.7, ncol=3)

    # Info panel
    yp = 0.97
    lh = 0.028

    ax_info.text(0.02, yp, f'{controller_label.upper()}  [{topology_name.upper()}]',
                 transform=ax_info.transAxes, fontsize=14, fontweight='bold', va='top')
    yp -= 0.05

    true_text = ax_info.text(0.02, yp, '', transform=ax_info.transAxes,
                             fontsize=10, fontfamily='monospace', va='top',
                             color=target_color, fontweight='bold')
    yp -= lh * 1.3

    cons_text = ax_info.text(0.02, yp, '', transform=ax_info.transAxes,
                             fontsize=10, fontfamily='monospace', va='top',
                             color=consensus_color, fontweight='bold')
    yp -= lh * 1.5

    ax_info.plot([0.02, 0.98], [yp + 0.005, yp + 0.005],
                 transform=ax_info.transAxes, color='#CCCCCC', linewidth=0.8, clip_on=False)
    yp -= 0.015

    ax_info.text(0.02, yp, 'LOCAL ESTIMATES', transform=ax_info.transAxes,
                 fontsize=10, fontweight='bold', va='top', color='#555555')
    yp -= lh * 1.2
    ...
    for i in range(num_drones):
        c = drone_colors[i % len(drone_colors)]
        txt = ax_info.text(0.02, yp, '', transform=ax_info.transAxes,
                           fontsize=9, fontfamily='monospace', va='top', color=c)
        local_texts.append(txt)
        yp -= lh
    ...
    ax_info.text(0.02, yp, 'TRACKING METRICS', transform=ax_info.transAxes,
                 fontsize=10, fontweight='bold', va='top', color='#555555')
    yp -= lh * 1.2

    trP_text = ax_info.text(0.02, yp, '', transform=ax_info.transAxes,
                            fontsize=10, fontfamily='monospace', va='top',
                            color='#333333', fontweight='bold')
    yp -= lh * 1.2

    cons_err_text = ax_info.text(0.02, yp, '', transform=ax_info.transAxes,
                                  fontsize=10, fontfamily='monospace', va='top',
                                  color=consensus_color)
    yp -= lh

    angle_text = ax_info.text(0.02, yp, '', transform=ax_info.transAxes,
                              fontsize=10, fontfamily='monospace', va='top',
    ...
    ax_info.text(0.02, yp, 'OBSERVERS', transform=ax_info.transAxes,
                 fontsize=10, fontweight='bold', va='top', color='#555555')
    yp -= lh * 1.2
    ...
    detect_text = ax_info.text(0.02, yp, '', transform=ax_info.transAxes,
                               fontsize=11, fontfamily='monospace', va='top',
                               fontweight='bold')

    # Controls
    rl_state = {"playing": True, "speed": 1.0, "frame": 0}

    ax_slider = fig.add_axes([0.08, 0.08, 0.84, 0.03])
    rl_slider = Slider(ax_slider, 'Time', 0, T - 1, valinit=0, valstep=1, color='#4A90D9')

    btn_h = 0.045
    btn_y = 0.015
    ax_play = fig.add_axes([0.08, btn_y, 0.10, btn_h])
    rl_btn_play = Button(ax_play, 'Pause', color='#E8E8E8', hovercolor='#D0D0D0')
    ax_restart = fig.add_axes([0.20, btn_y, 0.10, btn_h])
    rl_btn_restart = Button(ax_restart, 'Restart', color='#E8E8E8', hovercolor='#D0D0D0')
    ax_slow = fig.add_axes([0.42, btn_y, 0.08, btn_h])
    rl_btn_slow = Button(ax_slow, '0.5x', color='#E8E8E8', hovercolor='#D0D0D0')
    ax_normal = fig.add_axes([0.51, btn_y, 0.08, btn_h])
    rl_btn_normal = Button(ax_normal, '1x', color='#C8D8E8', hovercolor='#D0D0D0')
    ax_fast = fig.add_axes([0.60, btn_y, 0.08, btn_h])
    rl_btn_fast = Button(ax_fast, '2x', color='#E8E8E8', hovercolor='#D0D0D0')
    ax_vfast = fig.add_axes([0.69, btn_y, 0.08, btn_h])
    rl_btn_vfast = Button(ax_vfast, '4x', color='#E8E8E8', hovercolor='#D0D0D0')

    def _compute_angular_sep(t):
        tgt = target_traj[t]
        vecs = []
        for i in range(num_drones):
            d = drone_positions[t, i] - tgt
        ...
        for i in range(len(vecs)):
            for j in range(i + 1, len(vecs)):
                cos_a = np.clip(np.dot(vecs[i], vecs[j]), -1, 1)
    ...
    def _rl_render(t):
        ...
        for i in range(num_drones):
            pos = drone_positions[t, i]
            obs_dots[i].set_data_3d([pos[0]], [pos[1]], [pos[2]])
            sl = drone_positions[start:t+1, i]
            obs_trails[i].set_data_3d(sl[:, 0], sl[:, 1], sl[:, 2])

        for li, (i, j) in enumerate(comm_pairs):
            pi, pj = drone_positions[t, i], drone_positions[t, j]
            comm_lines[li].set_data_3d([pi[0], pj[0]], [pi[1], pj[1]], [pi[2], pj[2]])
            if active_edges is not None and t < len(active_edges):
                if active_edges[t][i, j] > 0:
                    comm_lines[li].set_color(comm_color)
                    comm_lines[li].set_linestyle('-')
                    comm_lines[li].set_alpha(0.6)
                    comm_lines[li].set_linewidth(2.0)
        ...
        target_dot.set_data_3d([tgt[0]], [tgt[1]], [tgt[2]])
        sl = target_traj[start:t+1]
        target_trail_line.set_data_3d(sl[:, 0], sl[:, 1], sl[:, 2])

        co = consensus_est[t, :3]
        cons_dot.set_data_3d([co[0]], [co[1]], [co[2]])
        sl = consensus_est[start:t+1, :3]
        cons_trail.set_data_3d(sl[:, 0], sl[:, 1], sl[:, 2])
        ...
        for i in range(num_drones):
            ...
            local_texts[i].set_text(
                f'D{i} x:{le[0]:7.1f} y:{le[1]:7.1f} z:{le[2]:7.1f} err:{le_err:5.1f}m')
        trP_val = tr_P_history[t] if t < len(tr_P_history) else 0.0
        trP_text.set_text(f'tr(P_pos): {trP_val:10.1f}')
        cons_err_text.set_text(f'Consensus err: {co_err:8.2f} m')
        ang_sep = _compute_angular_sep(t)
        angle_text.set_text(f'Angular sep:  {ang_sep:6.1f} deg')
        for i in range(num_drones):
            pos = drone_positions[t, i]
            dist = np.linalg.norm(pos - tgt)
            obs_texts[i].set_text(
    ...
    def rl_on_slider(val):
        rl_state["frame"] = int(val)
        _rl_render(rl_state["frame"])

    def rl_on_play(event):
        rl_state["playing"] = not rl_state["playing"]
        rl_btn_play.label.set_text('Play' if not rl_state["playing"] else 'Pause')
        fig.canvas.draw_idle()

    def rl_on_restart(event):
        rl_state["frame"] = 0
        rl_state["playing"] = True
        rl_btn_play.label.set_text('Pause')
        rl_slider.set_val(0)

    def rl_on_speed(speed):
        def handler(event):
            rl_state["speed"] = speed
            for ax_btn, s in [(ax_slow, 0.5), (ax_normal, 1.0), (ax_fast, 2.0), (ax_vfast, 4.0)]:
                ax_btn.set_facecolor('#C8D8E8' if s == speed else '#E8E8E8')
            fig.canvas.draw_idle()
        return handler

    rl_slider.on_changed(rl_on_slider)
    rl_btn_play.on_clicked(rl_on_play)
    rl_btn_restart.on_clicked(rl_on_restart)
    rl_btn_slow.on_clicked(rl_on_speed(0.5))
    rl_btn_normal.on_clicked(rl_on_speed(1.0))
    rl_btn_fast.on_clicked(rl_on_speed(2.0))
    rl_btn_vfast.on_clicked(rl_on_speed(4.0))

    def rl_update(frame_arg):
        if not rl_state["playing"]:
            return []
        step = max(1, int(rl_state["speed"]))
        rl_state["frame"] = (rl_state["frame"] + step) % T
        rl_slider.set_val(rl_state["frame"])
        _rl_render(rl_state["frame"])
        return []

    rl_anim = FuncAnimation(fig, rl_update, frames=None, interval=interval_ms,
                            blit=False, cache_frame_data=False)
