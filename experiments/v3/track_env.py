"""V3 Phase 2: TrackEnv — tracking with filter, starting from pre-spread positions.

Almost identical to V2TrackingEnv, with one key difference: drones can start
from pre-spread positions (Phase 1 output) instead of cluster/normal spawn.
"""
...
class TrackEnv(gym.Env):
    """Phase 2 environment: tracking with filter from pre-spread positions.

    Actor obs (14D per drone): same as V2
    Critic obs (23D per drone): actor obs + global filter state
    Action (3D per drone): residual correction to chase+offset
    Reward: PBRS with FIM potential (same as V2)
    """

    def __init__(self, cfg: V3Config, seed: int | None = None,
    ...
    def _chase_controller(self, drone_positions: np.ndarray) -> np.ndarray:
        """Base chase+offset controller (same as V2)."""
    ...
    def _build_obs(self, result: dict) -> np.ndarray:
        """Build 23D observation (14D actor + 9D critic extra). Same as V2."""
        obs = np.zeros((self.N, self.critic_obs_dim), dtype=np.float32)
    ...
    def _compute_reward(self, result: dict) -> np.ndarray:
        """PBRS reward (same as V2)."""
        if not self._filter.initialized:
            return np.full(self.N, -1.0, dtype=np.float32)

        tr_P = self._get_tr_P_pos()
        log_norm = np.log(1000.0 + 1.0)
        ...
        phi_new = np.clip(np.log(det_new), -20.0, 5.0)
        phi_old = np.clip(np.log(det_old), -20.0, 5.0)
        r_shaping = self.cfg.gamma * phi_new - phi_old
